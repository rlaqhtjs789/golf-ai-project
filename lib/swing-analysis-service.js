...
class SwingAnalysisService {
  ...
  async _sendShotData(shotData) {
    try {
      ...
      console.log(`   │ Trajectory Points: ${payload._Positions.length}`);
      console.log('   └─────────────────────────────────────\n');
  ...
  async _sendAnalysisResults(results) {
    try {
      ...
      if (this.mainWindow && this.mainWindow.webContents) {
        ...
        console.log('   ✅ Analysis results sent to renderer');
        if (results.front) {
      ...
      console.error('   ❌ Failed to send analysis results:', error.message);
      throw error;
    }
  }
