...
class SensorMonitor extends EventEmitter {
  constructor(config = {}) {
    ...
    this.firstShotVideos = []; // 첫 번째 샷용 영상 저장 (front/side)
  ...
  /**
   * 비디오 파일명 파싱
   * 파일명 끝의 f.mp4 또는 s.mp4로만 영상 타입 확인 (타임스탬프 무시)
   * 예: 250628075137,2313E6C25D6B,s,pyc80bsj,I7,...,f.mp4
   * 예: 250628075137,2313E6C25D6B,f,pyc80bsj,I7,...,s.mp4
   */
  _parseVideoFilename(filename) {
    // 파일명 끝에 ,f.mp4 또는 ,s.mp4가 있는지 확인
    const endMatch = filename.match(/,(f|s|club)\.mp4$/);
    if (!endMatch) {
      console.log(`   [Parse] Failed: File does not end with ,f.mp4 or ,s.mp4`);
      return null;
    }

    const typeChar = endMatch[1];
    const type = typeChar === 'f' ? 'front' : typeChar === 's' ? 'side' : 'impact';
    
    // 타임스탬프는 파일명의 첫 번째 필드에서 추출 (매칭에는 사용하지 않음)
    const parts = filename.split(',');
    const timestamp = parts[0] || Date.now().toString();

    console.log(`   [Parse] Success: type=${type}, timestamp=${timestamp} (timestamp not used for matching)`);
  ...
  /**
   * 대기 중인 비디오 추가
   * 첫 번째 영상(front/side)은 첫 번째 샷과 자동 매칭
   */
  _addPendingVideo(videoInfo, filename) {
    // 첫 번째 샷용 영상으로 저장 (front/side만)
    if (videoInfo.type === 'front' || videoInfo.type === 'side') {
      // 이미 같은 타입의 영상이 있으면 교체
      const existingIndex = this.firstShotVideos.findIndex(v => v.type === videoInfo.type);
      if (existingIndex >= 0) {
        this.firstShotVideos[existingIndex] = videoInfo;
        console.log(`   [First Shot Video] ${videoInfo.type} video updated (total: ${this.firstShotVideos.length})`);
      } else {
        this.firstShotVideos.push(videoInfo);
        console.log(`   [First Shot Video] ${videoInfo.type} video added (total: ${this.firstShotVideos.length})`);
      }
      
      // 첫 번째 샷이 이미 처리되었지만 영상이 없었던 경우, 영상 분석을 위해 이벤트 발생
      if (this.processedShotCount === 1 && this.firstShotVideos.length > 0) {
        console.log(`   [First Shot Video] First shot already processed, but video arrived later - triggering video analysis event`);
        const videos = {
          front: this.firstShotVideos.find(v => v.type === 'front') || null,
          side: this.firstShotVideos.find(v => v.type === 'side') || null,
          impact: this.firstShotVideos.find(v => v.type === 'impact') || null,
        };
        
        // 영상만 있는 이벤트 발생 (샷 데이터는 이미 처리됨)
        if (videos.front || videos.side) {
          console.log(`   [First Shot Video] Emitting video-only event for first shot analysis`);
          this.emit('firstShotVideoArrived', {
            videos: videos,
            shotNumber: 1,
            isFirstShot: true,
          });
  ...
  /**
   * 대기 중인 샷 데이터 추가
   * 첫 번째 샷이면 첫 번째 영상과 자동 매칭, 그 외는 영상 없이 처리
   */
  _addPendingShotData(shotData, filename) {
    ...
    console.log(`[Sensor] First shot videos available: ${this.firstShotVideos.length} (${this.firstShotVideos.map(v => v.type).join(', ') || 'none'})`);
    
    const isFirstShot = this.processedShotCount === 0;
    const key = shotData.timestamp ? shotData.timestamp.toString() : Date.now().toString();
    
    // 첫 번째 샷이면 첫 번째 영상과 매칭
    if (isFirstShot && this.firstShotVideos.length > 0) {
      const videos = {
        front: this.firstShotVideos.find(v => v.type === 'front') || null,
        side: this.firstShotVideos.find(v => v.type === 'side') || null,
        impact: this.firstShotVideos.find(v => v.type === 'impact') || null,
      ...
      const videoArray = [videos.front, videos.side, videos.impact].filter(v => v !== null);
      this.pendingShots.set(key, { videos: videoArray, shotData });
      console.log(`   [Pending] (${key}): Videos ${videoArray.length} (first shot - auto matched), Shot Data ✓`);
      console.log(`   [Pending] Video details: front=${!!videos.front}, side=${!!videos.side}, impact=${!!videos.impact}`);
      
      // 첫 번째 영상 사용 후 초기화
      this.firstShotVideos = [];
    } else {
      // 두 번째 이후 샷은 영상 없이 처리
      this.pendingShots.set(key, { videos: [], shotData });
      if (isFirstShot) {
        console.log(`   [Pending] (${key}): Videos 0, Shot Data ✓ (first shot but no videos available - will wait for videos)`);
      } else {
        console.log(`   [Pending] (${key}): Videos 0, Shot Data ✓ (2nd+ shot - videos not required)`);
      }
    }
